...
### 调用地点：
ppp_ioctl：cmd为PPPIOCSPASS或PPPIOCSACTIVE时，调用该函数创建filter，挂上ppp->pass_filter或者active_filter。pass_filter和active_filter都会在ppp_send_frame，ppp_receive_nonmp_frame中使用BPF_PROG_RUN运行，返回值为0则前者通过，后者标记时间。
